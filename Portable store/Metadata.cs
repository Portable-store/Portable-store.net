...
namespace Portable_store
{
    public static class Metadata
    {
        ...
        internal static async Task<string?> Create_info_Async(Application_info_Model application_info, IProgress<Progress_info_Model>? progress = null, CancellationToken cancellationToken = default)
        {
        ...
        public static async Task<string?> Create_Async(Application_metadata_Model metadata, IProgress<Progress_info_Model> progress, CancellationToken cancellationToken = default)
        {
        ...
        /// <returns></returns>
        public static async Task<Application_metadata_Model?> Read_Async(string file_name, IProgress<Progress_info_Model>? progress = null, CancellationToken cancellationToken = default)
        {
            var progress_info = new Progress_info_Model(1)
            {
            ...
            if (File.Exists(file_name))
            {
                try
                {
                    ...
                    var metadata = JsonSerializer.Deserialize<Application_metadata_Model>(metadata_JSON, JSON_option);

                    if (metadata != null)
                    {
                        if (!metadata.Name.Contains('/'))
                            throw new Exception("Name should be as \"Author/Application_name\"");

                        if (string.IsNullOrEmpty(metadata.Display_name))
                ...
                {
                    progress?.Report(progress_info.Update($"Failed to read the metadata file: {ex.Message}.", 0));
                }
            }
            else
                progress?.Report( progress_info.Update("Failed to read {file_name}: The metadata file don't exist.", 0) );


            return null;
        }

        public static IEnumerable<Application_version_Model> Get_compatible_versions(in Application_metadata_Model metadata) =>
